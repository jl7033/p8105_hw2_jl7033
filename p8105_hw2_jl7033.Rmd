...
In order to make the star baker table more legible, I arranged the entries by series and then by episode. The mean age of the star bakers from series 5 through 10 was `r round(mean(star_baker_df$baker_age), 1)`, slightly lower than the mean age for all bakers, which was `r round(mean(gbb_df$baker_age), 1)`. There doesn't seem to be a clear pattern in terms of occupation (there's a wide variety, from builders to educators to marketers and more) and hometown. Being named Star Baker most frequently wasn't perfectly correlated with winning; Richard Burr earned the Star Baker title four times in Series 5, and yet still finished as the runner-up, even though the winner, Nancy Birtwhistle, was only awarded the title in Week 1. Ian Cumming won the title three times in a row early in Series 6 but finished with the same result as Burr (though the winner, Nadiya Hussain, also earned the Star Baker title three non-consecutive times later in the series).
